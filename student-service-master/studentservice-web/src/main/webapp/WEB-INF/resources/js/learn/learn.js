...
var learnGrid = new Ext.grid.GridPanel({
			id : 'learnGrid',
			plugins : [rowEditing],
			store : store,
			region : 'center',
...
learnGrid.getSelectionModel().on('selectionchange',
		function(selModel, selections) {
			learnGrid.down('#delete').setDisabled(selections.length === 0);
		});
...
var clearForm = function() {
	...
	learnForm.getForm().reset();
}

var queryForm = function() {
	Ext.Msg.alert('查询', '将开始执行查询！');
}
var learnForm = new Ext.form.FormPanel({
			title : '信息查询',
			width : 200,
			height : 200,
			frame : true,
